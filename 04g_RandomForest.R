...
dt_test <- dt_all[fold_random %in% folds_test, 
                  c(vars_toUseOriginal, "target"),
                  with = FALSE]
n_train <- length(dt_train)
n_test <- length(dt_test)
...
ranger_1 <- 
  ranger(fmla_1,
         data = dt_train[,  c('target', vars_toUseOriginal), with = FALSE],
         num.trees = 100,
         #mtry = 25,
         replace = TRUE,
         min.node.size = 10, 
...
pred_test <- predict(ranger_1, dt_test)$predictions
